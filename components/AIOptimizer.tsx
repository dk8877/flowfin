...
export const AIOptimizer: React.FC = () => {
    ...
    return (
        <div className="p-6 pt-6 lg:pt-10 space-y-8">
            <div className="bg-gradient-to-r from-amber-500 to-emerald-600 rounded-3xl p-6 text-neutral-950 relative overflow-hidden">
            ...
            {optimization.length > 0 && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
                    ...
                    {optimization.map((opt, i) => (
                        <div key={i} className="bg-neutral-900 border border-neutral-800 p-5 rounded-2xl flex flex-col gap-3">
                            <div className="flex justify-between items-center">
                                <div className="flex items-center gap-2 text-slate-200 font-bold">
                                    <TrendingDown className="text-red-400" size={20} />
                                    {opt.category}
                                </div>
                                <span className="px-2 py-1 bg-neutral-800 rounded-md text-xs text-neutral-400 border border-neutral-700">{opt.change}</span>
                            </div>
                            <div className="flex gap-3 bg-neutral-950/50 p-3 rounded-xl border border-neutral-800/50">
                                <ArrowRight className="text-emerald-500 shrink-0 mt-0.5" size={16} />
                                <p className="text-sm text-neutral-400 leading-relaxed">{opt.advice}</p>
                            </div>
                        </div>
                    ))}
                </div>
